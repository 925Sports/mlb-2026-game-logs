...
for team in teams:
    ...
    for player in roster:
        ...
        # Hitting logs
        try:
            data = statsapi.player_stats(player_id, group="hitting", type="gameLog", season=2026)
            for game in data.get('stats', [{}])[0].get('splits', []):
                stat = game.get('stat', {})
                all_rows.append({
                    'Player': player_name,
                    'Team': team_name,
                    ...
                    'AB': stat.get('atBats'), 'R': stat.get('runs'), 'H': stat.get('hits'),
                    '2B': stat.get('doubles'), '3B': stat.get('triples'), 'HR': stat.get('homeRuns'),
                    'RBI': stat.get('rbi'), 'BB': stat.get('baseOnBalls'), 'SO': stat.get('strikeOuts'),
        ...
        # Pitching logs
        try:
            data = statsapi.player_stats(player_id, group="pitching", type="gameLog", season=2026)
            for game in data.get('stats', [{}])[0].get('splits', []):
                stat = game.get('stat', {})
                all_rows.append({
                    'Player': player_name,
                    ...
                    'H': stat.get('hits'), 'R': stat.get('runs'), 'ER': stat.get('earnedRuns'),
                    'BB': stat.get('baseOnBalls'), 'SO': stat.get('strikeOuts'), 'HR': stat.get('homeRuns'),
                    'ERA': stat.get('era'), 'WHIP': stat.get('whip'),
                })
        except:
            pass

# Save to CSV (newest games first)
